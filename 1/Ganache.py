...
def fetch_ganache_data(start_block, end_block):
    # 设置代理
    proxy = {
    ...
    try:
        ...
        # 从 MongoDB 获取最后处理的区块号
        last_processed_block = get_last_processed_block()

        # 如果 start_block 小于已处理的最新区块号，调整为最新已处理区块号
        if last_processed_block and start_block <= last_processed_block:
            print(f"警告: start_block {start_block} 小于已处理的最后一个区块号 {last_processed_block}，跳过已处理的区块。")
            start_block = last_processed_block + 1  # 从最后一个未处理区块开始

        # 确保提供的 end_block 不超过最新区块号
        if end_block > latest_block_number:
            print(f"错误: 结束区块号 {end_block} 大于最新区块号 {latest_block_number}，修正为最新区块号。")
