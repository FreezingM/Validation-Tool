...
db = client["eos_all_blockchain"]  # 创建数据库
...
# 获取账户的RAM配额和使用情况
def get_account_ram_info(account_name):
    """获取账户的 RAM 配额和已使用情况"""
    url = f"{EOS_RPC_URL}/v1/chain/get_account"
    payload = {"account_name": account_name}
    response = requests.post(url, json=payload)
    if response.status_code == 200:
        account_data = response.json()
        ram_quota = account_data.get("ram_quota", 0)
        ram_usage = account_data.get("ram_usage", 0)
        return ram_quota, ram_usage
    print(f"无法获取账户 {account_name} 的 RAM 信息")
    return 0, 0

# 封装的爬取 EOS 链数据的函数
def fetch_eos_all_data(start_block, end_block):
    try:
        # 获取 EOS 最新区块号
        ...
        for block_num in range(start_block, end_block + 1):
            ...
            for account in unique_accounts:
                ...
                # 获取账户 RAM 信息
                ram_quota, ram_usage = get_account_ram_info(account)
...
if __name__ == '__main__':
    fetch_eos_all_data(422947404, 422947405)